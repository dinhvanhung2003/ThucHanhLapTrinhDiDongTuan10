...
// Lấy action từ slice
const { setName, setTasks, addTask, deleteTask, editTask } = taskSlice.actions;
...
function HomeScreen({ navigation }) {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', gap: 100 }}>
      <Text style={{ color: '#8353E2', fontWeight: 'bold', textAlign: 'center', fontSize: 25, textTransform: 'uppercase' }}>
        Manage your{'\n'}task
      </Text>
      <View style={{ flexDirection: 'row', alignItems: 'center', borderWidth: 1, borderColor: 'gray', borderRadius: 10, padding: 10 }}>
        ...
        <TextInput
          style={{ flex: 1, textAlign: 'center', paddingHorizontal: 40, outline: 'none' }}
      ...
      <TouchableOpacity
        style={{ padding: 10, paddingHorizontal: 40, backgroundColor: '#00BDD6', borderRadius: 10 }}
        onPress={() => navigation.navigate('Details')}
      >
        <Text style={{ color: 'white', textTransform: 'uppercase' }}>Get Started ➙</Text>
      </TouchableOpacity>
    </View>
  );
}
...
function AddTaskScreen({ navigation, route }) {
  ...
  const handleFinish = () => {
    ...
    method(url, { title: task })
      .then((response) => {
        if (taskToEdit) {
          dispatch(editTask({ taskId: taskToEdit.id, title: task }));
        } else {
